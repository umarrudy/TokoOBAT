...
class InitialSeeder extends Seeder
{
    ...
    {   
        ...
        DB::table('users')->insert([
            'name' => 'Super Admin',
            'email' => 'admin@tokobat.id',
            'password' => Hash::make('admintokobat'),
        ]);

        // Find newly created user
        $user = User::where('email', 'admin@tokobat.id')->first();

        // Create Staff User
        DB::table('users')->insert([
            'name' => 'Staff',
            'email' => 'staff@tokobat.id',
            'password' => Hash::make('stafftokobat'),
        ]);

        // Find newly created staff
        $userStaff = User::where('email', 'staff@tokobat.id')->first();
